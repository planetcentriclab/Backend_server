...
const select_waterFlow = (req, res) => {
    pool.query(querise.select_waterFlow , (error, result) => {
        if (error) throw error;
        res.status(200).json(result.rows)
    })
}
...
const newRecord = (req, res) => {
    ...
    pool.query(querise.newRecord_waterFlow, [create_date, create_time, waterFlow.value_raw, waterFlow.value_con])
